...
**Link the PR back to this task.** Whenever you open a pull request for this task (\`gh pr create\`), end the description with a deep link back to the task so reviewers can jump into it. Take \`<TASK_ID>\` from \`dev3 current\` and append this footer verbatim (GitHub strips \`dev3://\` links, so the \`https\` form is the clickable one and the raw link is a copy-paste fallback):
